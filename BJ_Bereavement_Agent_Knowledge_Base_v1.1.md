# 北京市居民身后事务 AI Agent｜可执行政策知识库 v1.1
...
本次修订针对内部审查中发现的问题，具体修复如下：

1. 修复 `RECOMMEND_CREMATION` 中隐藏的不可见字符（软连字符），此前可能导致 forbidden-action 字符串匹配失效。
2. 修复 `FUNERAL_002` 中 `task_id: TASK_CREATION` 拼写错误，更正为 `TASK_CREMATION`。
3. `Q_DEATH_LOCATION` 增加“外地”“境外”选项，并新增 `CROSS_REGION_001` 规则，衔接第 21 节“跨区域遗体运输”高风险域。
4. `CaseProfile.insurance` 增加 `pension_type_source` 字段（`OFFICIAL` / `INFERRED` / `UNKNOWN`），落实第 6 节“推断结果 ≠ 官方事实”的要求。
5. 新增 `DEFENSE_IN_DEPTH_001`：明确要求所有殡葬、户籍、养老、医保、公积金相关规则必须各自重新校验 `death_nature == NORMAL`，不得仅依赖上游一次性把关。
6. `DEATH_001` 补充明确的 Task 定义（`TASK_CONFIRM_DEATH`），此前该规则只有 Decision，没有可执行 Task，导致“确认死亡”这一步在下游系统里容易被忽略。
7. 新增 `ETHNIC_RELIGIOUS_001`：为“民族/宗教特殊丧葬权利”高风险域提供明确触发条件和分流动作。
8. 新增 `HEIR_DISPUTE_001`：为“继承资格/继承人争议/亲属关系争议”高风险域提供明确触发条件（多个可能继承人、办理人关系存疑等），并与 `family` 数据对象显式关联。
9. `HOUSING_FUND_001` 的 Task Chain 中 `HEIR_OR_LEGATEE_CONFIRMATION` 步骤标注为强制关联 `HEIR_DISPUTE_001` 高风险检查点。
10. Task Object 模板（第 15 节）与 Policy Record Template（第 19 节）明确：任何绑定官方政策依据的 Task，`official_source` 与 `last_verified` 不允许为空；纯导航类 Task（如确认死亡）可以豁免。同时 `materials` 结构从纯字符串数组改为带 `requirement_level`（`COMMON` / `MANDATORY`）的对象数组，落实第 25 节安全清单要求。
...
## DEFENSE_IN_DEPTH_001

**Purpose:** 防止安全闸门仅在单一环节把关、下游被绕过。

> v1.1 新增。此前 `SAFETY_ABNORMAL_001` 只在案件早期检查一次 `death_nature`，第 5 章及之后各 Domain 的规则（`FUNERAL_*` / `HOUSEHOLD_*` / `PENSION_*` / `MEDICAL_*` / `HOUSING_FUND_*`）在其 Preconditions 中大多只检查 `death_certificate` 等状态字段是否为真，没有重新校验死亡性质，一旦 `death_certificate` 字段在异常情况下被置为 `true`（例如数据回填错误、状态被绕过等），这些下游规则理论上会独立触发，绕开安全闸门。

### Rule

```text
本知识库第 5 章及之后所有涉及殡葬、户籍、养老保险、医保、住房公积金的
Executable Rule，其 Preconditions 必须显式包含：

    death_nature == NORMAL

不得仅依赖 SAFETY_ABNORMAL_001 或 DEATH_CERT_001 的一次性把关。
```

### Implementation Note

```text
Rule Engine 层面：SAFETY_ABNORMAL_001 的 priority 必须高于所有其他规则；
一旦 mode == ESCALATED 或 paused == true，Rule Engine 应停止评估所有非
SAFETY 类规则，而不仅仅依赖各条规则各自的 precondition 检查。
两者（规则级 precondition + 引擎级全局拦截）应同时具备，形成纵深防御。
```
...
### Task（not_confirmed 分支）

> v1.1 新增：此前 DEATH_001 只有 Decision，没有对应 Task，容易导致“确认死亡”这一步在下游系统实现时被跳过。

```yaml
task_id: TASK_CONFIRM_DEATH
priority: P0
status: ACTION_REQUIRED
title: 确认死亡状态
reason: 死亡确认是启动后续一切流程的前提，未确认前不得进入死亡性质判断或殡葬流程。
recommended_action: 联系死亡发生地的医疗机构（医院/养老机构/急救部门）确认死亡状态。
defer_allowed: false
official_source: null
last_verified: "2026-08-12"
```

> `official_source` 允许为空：本任务属于纯流程导航，不涉及具体政策条文引用。

### 跨区域/境外死亡提示

若 `Q_DEATH_LOCATION` 选择“北京市外地（外省市）”或“境外”，进入 `CROSS_REGION_001`（见第 21 节高风险域关联规则），Agent 仅作分流提示，不判断具体的跨区域遗体运输可行性或费用。
...
priority: P2
status: READY
official_source: 北京市殡葬管理条例 / 北京市民政局
last_verified: "2026-08-12"
```

> v1.1 变更：此前本条 Task 缺失 `official_source` / `last_verified`，违反第 25 节安全清单“所有核心政策绑定官方来源”的要求，现已补全。
...
推断结果必须写入 `CaseProfile.insurance.pension_type_source`：

```yaml
pension_type_source: INFERRED   # 用户明确回答时为 OFFICIAL；靠在职/单位性质等间接问题推断时为 INFERRED
```

> v1.1 变更：此前本规则只以文字约定“必须标记 source_type: INFERRED”，但 CaseProfile 数据模型中没有承载该标记的字段，导致标记无法真正落地。现已在 3.1 节 `insurance` 对象中加入 `pension_type_source` 字段。当 `pension_type_source == INFERRED` 时，Agent 在涉及该字段的所有下游话术中都应保留不确定性表达（例如“根据您的描述初步判断”），并在合适时机提示用户核实。
...
HOUSING_FUND_ACCOUNT_SEAL
    ↓
HEIR_OR_LEGATEE_CONFIRMATION   ← 强制触发 HEIR_DISPUTE_001 高风险检查（见第 21 节）
    ↓
HOUSING_FUND_WITHDRAWAL
```

> v1.1 变更：`HEIR_OR_LEGATEE_CONFIRMATION` 不得被下游系统简化为普通子任务直接放行。执行到此步骤时必须先运行 `HEIR_DISPUTE_001` 判断是否存在继承人资格争议，若存在则停止自主判断并转入高风险分流，而不是继续走 `HOUSING_FUND_WITHDRAWAL`。
...
```json
{
  ...
  "department": "公安机关",
  "materials": [
    { "name": "死亡证明", "requirement_level": "MANDATORY" },
    { "name": "户口簿", "requirement_level": "MANDATORY" },
    { "name": "死亡人员身份证", "requirement_level": "MANDATORY" },
    { "name": "办理人身份证", "requirement_level": "MANDATORY" },
    { "name": "关系证明（如经办窗口要求）", "requirement_level": "COMMON" }
  ],
  "prerequisites": [
    "death_certificate = true"
  ],
...
### Materials 字段规则（v1.1 新增）

```text
requirement_level: MANDATORY   → 该地区/情形下普遍要求的核心材料
requirement_level: COMMON      → 常见但可能因地区/情形不同而有出入的材料，
                                   Agent 表达时必须使用“通常需要”“可能需要”等
                                   不确定语气，不得承诺一致
```

### official_source / last_verified 强制性规则（v1.1 新增）

```text
若某 Task 的 recommended_action 涉及具体政策依据（金额、资格、材料清单等），
则 official_source 与 last_verified 不允许为 null。

仅当 Task 属于纯流程导航类（如 TASK_CONFIRM_DEATH 这种不依赖具体政策条文的
提示性任务）时，official_source 才允许为 null，但仍必须保留 last_verified
（记录知识库本条规则的最后核实时间，而非某项政策的核实时间）。
```
...
trigger_condition:
precondition:
death_nature_precondition: NORMAL   # v1.1 新增：除安全闸门/高风险分流类规则外，默认必须为 NORMAL
exclusion_condition:
...
materials:
  - name:
    requirement_level:   # MANDATORY | COMMON
...
> v1.1 变更：此前本节只是文字列出高风险域名单，没有配套的可执行触发条件，导致 Rule Engine 无法真正识别这些情形。以下补充三条具体的触发规则，均需 Case Parser 采集对应字段后才能生效。

## HEIR_DISPUTE_001｜继承资格与亲属关系争议

**Domain:** 高风险 / `继承资格`、`继承人争议`、`亲属关系争议`

### Trigger

```text
family.other_possible_heirs 非空且包含一个以上潜在继承人
或
family.handler_relation 与家属自述关系不一致 / 存疑
或
用户主动描述“家属对遗产/继承有分歧”“不确定谁是合法继承人”等情形
```
...
```text
停止自主判断继承资格与份额
仅提示：继承人范围与顺序、遗产分配以《中华人民共和国民法典》继承编等法律规定和/或遗嘱内容为准
推荐：公证处 / 律师 / 必要时通过司法途径解决
```

### Forbidden

```text
DETERMINE_HEIR_ELIGIBILITY
...
## ETHNIC_RELIGIOUS_001｜民族/宗教特殊丧葬权利

**Domain:** 高风险 / `民族/宗教特殊丧葬权利`

### Trigger

```text
deceased.ethnicity 或 deceased.religion 字段被填写为存在特殊丧葬习俗的民族/宗教
或
用户主动提出殡葬习俗方面的特殊要求（如土葬、特定殡葬仪式等）
```

### Action

```text
不由 Agent 自行判断该民族/宗教习俗在北京市的具体政策适用与例外
提示：北京市相关民族宗教事务主管部门、殡仪馆民族丧葬服务窗口
```

### Forbidden

```text
DETERMINE_RELIGIOUS_EXEMPTION
ADVISE_ON_BURIAL_METHOD_LEGALITY
```

---

## CROSS_REGION_001｜跨区域/境外死亡遗体运输
...
```text
Q_DEATH_LOCATION 回答为“北京市外地（外省市）”或“境外”
```

### Action

```text
不判断具体的跨省市/跨境遗体运输可行性、流程或费用
提示：需先联系死亡发生地相关部门（属地公安/卫生机构，境外情形另需联系我国驻当地使领馆）确认当地要求，
再与拟接收地（北京）殡仪机构沟通对接
```
...
```text
ARRANGE_CROSS_REGION_TRANSPORT
PROVIDE_CROSS_BORDER_LEGAL_ADVICE
```
...
以下为 v1.1 新增检查项：
[ ] 第 5 章及之后所有 Domain 规则的 Preconditions 均显式包含 death_nature == NORMAL（纵深防御，见 DEFENSE_IN_DEPTH_001）
[ ] Rule Engine 在 mode == ESCALATED / paused == true 时全局停止评估非 SAFETY 类规则，而不仅依赖单条规则的 precondition
[ ] “确认死亡”步骤（DEATH_001）有对应可执行 Task，不能只停留在 Decision 文本
[ ] 继承资格/亲属关系争议（HEIR_DISPUTE_001）、民族宗教特殊丧葬权利（ETHNIC_RELIGIOUS_001）、跨区域/境外遗体运输（CROSS_REGION_001）三条高风险规则均有明确的 Trigger 字段，能被 Rule Engine 实际识别，而非仅作为文字清单存在
[ ] materials 字段统一使用 { name, requirement_level } 结构，禁止退化为纯字符串数组
[ ] CaseProfile 中所有靠对话推断而非用户直接确认的字段（如 pension_type）均有配套的 *_source 字段记录 OFFICIAL / INFERRED
```
